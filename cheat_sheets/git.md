...
1. Create a "Fix" Branch. Always perform reverts on a separate branch. This allows for a Pull Request, enabling team review and CI checks.
...
1. Revert the Unwanted Commits

Create new commits that undo the changes from the old, unwanted ones.

```bash
# Revert specific commits. List their SHAs.
git revert <commit-hash-1>
git revert <commit-hash-2>
```

1. Resolving Conflicts (If They Occur)

a.  **Open the conflicted file.** You'll see markers:
...
c.  **Finalize the revert:**
    ...
    **Safety Escape Hatch:** If you get stuck, cancel everything.
    ```bash
    git revert --abort
    ```

1. Squash Reverts into a Single Commit
...
b.  **Edit the rebase plan.** Your editor will open. Change `pick` to `s` (squash) for all commits except the first one.
    ```
